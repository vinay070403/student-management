        ...
        <div class="tab-content" id="schoolTabsContent">
            ...
            <div class="tab-pane fade show active" id="info" role="tabpanel">
                <form action="{{ route('schools.update', $school->id) }}" method="POST" class="row g-4">
                    ...
                    <div class="col-md-6">
                        <label for="name" class="form-label fw-bold text-dark">School Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control form-control-lg rounded-3 shadow-sm" value="{{ $school->name }}" required>
                    </div>

                    <div class="col-md-6">
                        <label for="state_id" class="form-label fw-bold text-dark">State Name <span class="text-danger">*</span></label>
                        <select name="state_id" class="form-select form-select-lg rounded-3 shadow-sm select2" required>
                        ...
                        <div id="grade-scale-wrapper">
                            ...
                            <div id="grade-rows">
                                ...
                                <div class="row align-items-center mb-3 grade-row bg-white p-2 rounded-3 shadow-sm">
                                    ...
                                    <div class="col-md-2">
                                        <input type="text" name="grade[]" class="form-control form-control-sm fw-bold text-center" value="{{ $grade->grade }}" placeholder="Grade" required>
                                    </div>
                                    <div class="col-md-3">
                                        <select name="min[]" class="form-select form-select-sm text-center" required>
                                            <option value="">Select</option>
                                            @for($i = 0; $i <= 100; $i++)
                                                <option value="{{ $i }}" {{ $grade->min_score == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                @endfor
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <select name="max[]" class="form-select form-select-sm text-center" required>
                                            <option value="">Select</option>
                                            @for($i = 0; $i <= 100; $i++)
                                                <option value="{{ $i }}" {{ $grade->max_score == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                @endfor
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                            ...
                            <button type="button" id="add-grade-btn" class="btn btn-dark mt-3 fw-bold shadow-sm">+ Add Grade</button>
                        </div>
                    </div>

                    <div class="col-12 text-end mt-4">
                        <button type="submit" class="btn btn-dark px-5 py-3 rounded-3 btn-lg fw-bold shadow-sm">Save Changes</button>
                    </div>
                </form>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <div class="col-md-2 text-end">
            <button type="button" class="btn btn-danger btn-sm remove-grade-btn">Delete</button>
        </div>`;
        ...
        function validateAll() {
            ...
            const minInputs = Array.from(document.querySelectorAll('input[name="min[]"], select[name="min[]"]'));
            const maxInputs = Array.from(document.querySelectorAll('input[name="max[]"], select[name="max[]"]'));
